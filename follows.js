...
//					if(duration == '6hours') diff = 21600000;
					...
					if(updated.getTime() > then.getTime()) {         //last updated data less than 1 week ago
						if(datastreamIds && datastreamIds != '' && datastreamIds.indexOf(datastream.id) >= 0) {      //correct datastream identified
							...
							xively.datastream.history(feedId, datastream.id, {start=2016-05-20T11:01:46Z&duration=month, limit: 1000}, function(datastreamData) {       //puts data in datastreamData duration: '2weeks', interval: '1800' works
								var series = [];
								var points = [];
